...
public class P746MinCostClimbingStairs {
    ...
    public static int minCostClimbingStairs(int[] cost) {
        int secondLastStepCost = cost[0];
        int lastStepCost = cost[1];

        for (int step = 2; step < cost.length; step++) {
            int stepCost = cost[step] + Math.min(lastStepCost, secondLastStepCost);
            secondLastStepCost = lastStepCost;
            lastStepCost = stepCost;
        }

        return Math.min(lastStepCost, secondLastStepCost);
    }
